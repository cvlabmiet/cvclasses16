...
void ObjectTrackingLK::Run(cv::VideoCapture & capture)
{
	...
	background = cv::imread("Bryansk.png", CV_LOAD_IMAGE_GRAYSCALE);
	if (background.empty())
	{
	...
	cv::VideoWriter outputVideo("lab7_out.avi", ex, fps, S);
	...
	for (;;)
	{
		...
		if (needToInit)
		{
			//cv::imshow("Randomnoe", foreground);
		...
		else if (!points[0].empty())
		{
			...
			for (size_t i = 0; i < points[1].size(); i++)
			{
				...
				if (sqrt(pow(dx, 2.0) + pow(dy, 2.0)) < 0.1)
				{
