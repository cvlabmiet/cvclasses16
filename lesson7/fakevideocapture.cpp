...
cv::Mat FakeVideoCapture::next_frame()
{
    ...
    const double timeToPause = 20;
    const double timeToResume = 30;
    const double travelTime = 30;
    ...
    if (time < timeToAppear) {
        ...
        pauseCenterX = centerX;
        pauseCenterY = centerY;
    }
    else if (time < timeToResume) {
        draw_shape(frame, shape, cv::Point2i(pauseCenterX, pauseCenterY), color);
//        draw_shape(motionBinary, shape, cv::Point2i(centerX, centerY), cv::Scalar(255));
    }
    else {
        // draw shape
        ...
        int centerX = static_cast<int>(width * (time - timeToAppear - (timeToResume - timeToPause)) / travelTime);
        int centerY = static_cast<int>(constant + amp * sin(freq * centerX));
